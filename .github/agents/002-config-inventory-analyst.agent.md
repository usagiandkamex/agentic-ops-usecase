...
- **EOL 横断照合（全成分・Azure サービス・CVE と対の必須処理）**: サポート終了（EOL / 製品ライフサイクル）を次の **5 系統**すべてで照合し `vulnerabilities[]`（findingType=EOL）に格納する。**① OS**（`runtimeInventory[]` 区分 `ospackage` / `inventory[].runtime`）・**② DB エンジン**（PaaS マネージド DB・DB on VM。区分 `dbengine`）・**③ ランタイム**（App Service/Functions/Container Apps・AKS の K8s 版数。区分 `runtime`）・**④ パッケージ / ミドルウェア**（MDVM softwareinventories。区分 `language` / `middleware`）は、`runtimeInventory[]` の**全成分**を **製品×版数で重複排除**して endoflife.date（主）/ MicrosoftLifecycle（補助）/ DistroSecurityTracker で照合する（照会は製品×版数単位で一意化し、所見は **リソース×コンポーネント×製品×版数**で格納して別コンポーネントを畳み込まない）。**⑤ Azure マネージドサービスのリタイア**（例: PostgreSQL/MySQL Single Server 廃止・VM シリーズ廃止・AKS バージョンのサポート終了・API 版数の廃止）は `inventory[]`（`resourceType` を主キーに、利用可能なら API 版数 / SKU / デプロイメントモデル）から一意化し、**Azure Updates（Microsoft Release Communications）を MRC MCP サーバ（`https://www.microsoft.com/releasecommunications/mcp`・Azure/MCP ツールを優先）／RSS・API（`https://www.microsoft.com/releasecommunications/api/v2/azure/rss`。retirements はカテゴリ・表題で抽出）で機械照会**（補助 endoflife.date の Azure 系 / MicrosoftLifecycle）して `component=AzureService`・`source=AzureRetirement` で格納する。**EOL 到達（EOL 日付 ≤ 分析日。Azure のリタイア日も EOL 日付として同様に扱う）／終了間近**は〈参照 B〉で判定し、深刻度を持たないため `severity=null`。**該当版数が影響を受けるものだけ**を載せ、推測 URL・未確認情報は作らない（取得不可は「取得不可」と明示）。①〜④ は CVE と同じく〈参照 N〉の research サブエージェント（`azure-cve-lookup-worker`＝CVE+EOL 統合）に委譲でき、⑤ の Azure リタイアは上記 MRC MCP／Azure Updates RSS で**機械照会**でき、**親 Agent がオーケストレート**する（MCP・RSS とも到達不可のときのみ `downgraded` で取得不可を明示。`EOL:lookup` / `EOL:azureService` タスクを消化・〈参照 J〉）。
...
- (18) **一時ファイルなし**: レポートフォルダに `temp-*.*`（一時 JSON/TXT）や `findings-*.json`（別名）が 0 件で、成果物が HTML 3 / CSV 4 / `findings.json` / `progress.md` のみ。
  ...
  # done ⟺ 対象配列が非空（相互チェック）。authoritativeResourceEnumeration→inventory / softwareinventories→runtimeInventory / assessments→(securityRecommendations+vulnerabilities) / patchassessmentresources→patchAssessment / CVE:runtimeLookup・CVE:osLookup→公開ソース由来 CVE（findingType=CVE かつ source∈NVD/MITRE/GHSA/DistroSecurityTracker/MSRC）/ EOL:lookup→非 Azure の EOL（findingType=EOL かつ component≠AzureService）/ EOL:azureService→Azure サービスの EOL（findingType=EOL かつ component=AzureService）。EOL は component で排他に割り当て、別タスクの所見で誤って合格しないようにする（常時タスク・〈参照 J-1〉）
  ...
  $eolRun=($j.vulnerabilities | Where-Object { $_.findingType -eq 'EOL' -and $_.component -ne 'AzureService' } | Measure-Object).Count  # ①〜④（OS/DB/ランタイム/パッケージ・ミドルウェア）の EOL 反映件数
  $eolAz=($j.vulnerabilities | Where-Object { $_.findingType -eq 'EOL' -and $_.component -eq 'AzureService' } | Measure-Object).Count  # ⑤ Azure サービスのリタイア反映件数
