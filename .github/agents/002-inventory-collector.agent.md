...
- **所見の深刻度は必ず確定（空/`null` 禁止・JSON 作成側で対応）**: `vulnerabilities[]` の**全要素**の `severity` を **`Critical`/`High`/`Medium`/`Low` のいずれか**に確定する（情報源から得られない場合も空にせず下記の決定論フォールバックで埋める＝remediation/index の深刻度列を空にしない）。決定論の割り当て:
  - `CVE`: 情報源の CVSS（Critical/High）をそのまま採用。CVSS が取得できない場合のみ既定 `High`（CVE の掲載基準が Critical/High のための床値。スコア値の捏造はしない）。
  - `EOL`: 到達済み（EOL 日付 ≤ 分析日）=`High` / 終了間近（分析日から 365 日以内）=`Medium`（CVSS を持たないため決定論付与）。
  - `PatchMissing`: 未適用 Critical パッチあり=`High` / Security パッチのみ=`Medium` / その他更新のみ=`Low`（分類不明時は `Medium`）。
  情報源が値を持たない項目は**推測 CVE/CVSS スコアを創作せず**、上記の表示用深刻度ランクを決めるだけに留める（格上げの捏造はしないが空にもしない）。
