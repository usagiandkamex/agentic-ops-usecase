...
- **担当レイヤの特定（AND の非対称性）**: 許可したい（現状 Denied）→総合を Allowed にするには**すべての Deny レイヤ**を変える必要があるため、**全 Deny レイヤを担当レイヤとして列挙し全レイヤ分の提案を出す**（1 レイヤだけでは通らない）。拒否したい（現状 Allowed）→いずれか 1 レイヤで拒否すれば塞げるので、最小変更で安全に塞げるレイヤを挙げる。
